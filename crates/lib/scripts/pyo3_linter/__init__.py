"""
A lint helper for ``qcs-sdk``'s PyO3 wrappers.
...
def find_possible_mistakes(
    ...
    If you're using this to lint code other than ``qcs_sdk``, you'll probably want to change them,
    but keep in mind, this linter is designed with consideration of macros used in ``qcs_sdk``.
    If they are not given, it uses these defaults:

    - "builtins": This is implicitly "exported".
    - "qcs_sdk": This is the name of the top-level package, so it doesn't need an export.
    - "_qcs_sdk": This is the name of the internal package produced by the build process,
        and its contents are exported as the top-level package within the ``__init__.py``.

    The split between the "internal" ``_qcs_sdk`` and "external" ``qcs_sdk`` packages
    ...
    if found_exports is None:
        found_exports = {"builtins", "qcs_sdk", "._qcs_sdk"}
