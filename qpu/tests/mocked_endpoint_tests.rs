...
use qcs_util::{SECRETS_PATH_VAR, SETTINGS_PATH_VAR};
use qpu::{run_program, QPUResult};
...
async fn successful_bell_state() {
    setup().await;
    let result = run_program(BELL_STATE, 2, "ro", QPU_ID)
        .await
        .expect("Failed to run program that should be successful");
    assert_eq!(result[0], QPUResult::I8(vec![0, 1]));
    assert_eq!(result[1], QPUResult::I8(vec![0, 1]));
}
